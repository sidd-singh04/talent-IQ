...
// Map frontend language keys to JDoodle language & versionIndex
const JDoodleLangMap = {
...
router.post("/", async (req, res) => {
  try {
    ...
    if (!JDoodleLangMap[language]) return res.status(400).json({ error: "Unsupported language" });
    ...
    const jdResponse = await fetch("https://api.jdoodle.com/v1/execute", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
    });
    ...
    const text = await jdResponse.text();
    if (!text) return res.status(500).json({ error: "JDoodle returned empty response" });

    let data;
    try { data = JSON.parse(text); } 
    catch (e) {
      return res.status(500).json({ error: "Invalid JSON from JDoodle", details: text });
    }

    if (data.error) return res.status(500).json({ error: data.error, output: data.output || "" });

    return res.status(200).json({ success: true, output: data.output || "No output" });

  } catch (error) {
    console.log("Execution error:", error);
    return res.status(500).json({ error: "Code execution failed", details: error.message });
  }
});
